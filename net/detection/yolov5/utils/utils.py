...
def check_file(in_file):
    ...
    if os.path.isfile(in_file):
        ...
        print('iii', in_file)
        files = glob.glob('./**/' + in_file, recursive=True)  # find file
        print(files)
        assert len(files), 'File Not Found: %s' % in_file  # assert file was found
...
def box_iou(box1, boxs, iou_thre):
    match_results = []
    for index in range(box1.shape[0]):
        match_index = []
        ixmin=np.maximum(boxs[:, 0], float(box1[index][0]))
        iymin=np.maximum(boxs[:, 1], float(box1[index][1]))
        ixmax=np.minimum(boxs[:, 2], float(box1[index][2]))
        iymax=np.minimum(boxs[:, 3], float(box1[index][3]))
        iw=np.maximum(ixmax-ixmin+1,0)
        ih=np.maximum(iymax-iymin+1,0)
        inters=iw*ih
        uni= (float(box1[index][2]) - float(box1[index][0]) + 1) * (float(box1[index][3]) - float(box1[index][1]) + 1)
        uni+= (boxs[:, 3] - boxs[:, 1] + 1) * (boxs[:, 2] - boxs[:, 0] + 1) - inters
        overlaps=inters/uni
        for i in range(len(overlaps)):
            if overlaps[i] > iou_thre:
